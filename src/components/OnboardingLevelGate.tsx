...
export default function OnboardingLevelGate({ onFinish, appName = "LinguaPath" }) {
  ...
  if (step === "welcome") return <div style={card}>
    ...
    <div style={{fontFamily:"var(--font-display)",fontWeight:900,fontSize:40,lineHeight:1.05,marginBottom:10}}>Are you new to German?</div>
    <div style={{fontSize:17,lineHeight:1.75,color:"rgba(74,40,0,0.7)",maxWidth:680,marginBottom:24}}>Choose the smoothest starting point. Beginners go straight into A1. If you already know some German, take a short placement check so the app can recommend the best level.</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16}}>
      <button onClick={() => onFinish({ isBeginner:true, placedLevel:"A1", freeAccessLevel:"A1", previewLessonsAllowed:0, placementScore:0 })} style={btnBase}>
  ...
  if (step === "placement") return <div style={card}>
    ...
    <div style={{fontFamily:"var(--font-display)",fontWeight:900,fontSize:34,lineHeight:1.08,marginBottom:10}}>Let’s find your German level</div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14,maxHeight:"62vh",overflowY:"auto",paddingRight:4}}>
      {PLACEMENT_QUESTIONS.map((question, index) => <div key={question.id} style={{border:"1.5px solid rgba(245,165,36,0.18)",borderRadius:20,padding:18,background:"rgba(255,255,255,0.62)"}}>
        ...
        <div style={{display:"grid",gap:10}}>{question.options.map((option, optionIndex) => {
          const selected = answers[question.id] === optionIndex;
          return <button key={option} onClick={() => setAnswers(prev => ({...prev, [question.id]: optionIndex}))} style={{textAlign:"left",borderRadius:16,padding:"13px 14px",border:selected?"1.5px solid #f5a524":"1.5px solid rgba(245,165,36,0.16)",background:selected?"rgba(245,165,36,0.12)":"rgba(255,255,255,0.75)",color:"#4a2800",fontWeight:700,cursor:"pointer"}}>{option}</button>;
        })}</div>
      </div>)}
    </div>
  ...
  return <div style={card}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:14}}><div style={{width:76,height:76,borderRadius:22,background:"rgba(245,165,36,0.12)",border:"1.5px solid rgba(245,165,36,0.22)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:42,fontWeight:900,color:"#f5a524"}}>{placedLevel}</div><div style={{fontSize:16,lineHeight:1.7,color:"rgba(74,40,0,0.72)"}}>{placedLevel === "A1" ? "You can begin A1 immediately on the free plan." : `You can preview ${FREE_PLAN_RULES.previewLessonsForPlacedUsers} lessons in ${placedLevel} on the free plan. Full progression in ${placedLevel} and above is part of Pro.`}</div></div>
    <div style={{padding:18,borderRadius:18,background:"rgba(255,255,255,0.62)",border:"1.5px solid rgba(245,165,36,0.16)",marginBottom:16}}><div style={{fontSize:12,color:"rgba(74,40,0,0.55)",marginBottom:4}}>Placement score</div><div style={{fontSize:30,fontWeight:900}}>{correctAnswers}/{PLACEMENT_QUESTIONS.length}</div></div>
    <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:18}}>{LEVEL_ORDER.map(level => <div key={level} style={{padding:"8px 14px",borderRadius:999,background:level===placedLevel?"#f5a524":"rgba(245,165,36,0.08)",color:level===placedLevel?"#fff":"#8a5b10",fontWeight:800,fontSize:13}}>{level}</div>)}</div>
    <div style={{display:"flex",gap:10}}><button onClick={() => setStep("welcome")} style={{padding:"12px 18px",borderRadius:999,border:"1.5px solid rgba(245,165,36,0.18)",background:"transparent",cursor:"pointer",color:"#4a2800",fontWeight:700}}>← Back</button><button onClick={() => onFinish({ isBeginner:false, placedLevel, freeAccessLevel: placedLevel === "A1" ? "A1" : placedLevel, previewLessonsAllowed: placedLevel === "A1" ? 0 : FREE_PLAN_RULES.previewLessonsForPlacedUsers, placementScore: correctAnswers })} style={{padding:"13px 24px",borderRadius:999,border:"none",background:"#f5a524",color:"#fff",fontWeight:800,cursor:"pointer"}}>Continue →</button></div>
  </div>;
}
